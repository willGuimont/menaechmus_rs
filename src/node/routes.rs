...
#[get("/")]
pub async fn get_blockchain(node_config: &State<NodeConfig>) -> Json<BlockchainDto<ContentType>> {
    // let node = node_config.inner().0.lock().await;
...
pub async fn sync_blockchain(node_config: &State<NodeConfig>, blockchain: Json<BlockchainDto<ContentType>>) {
...
pub async fn add_mined_block(node_config: &State<NodeConfig>, block: Json<MinedBlockDto<ContentType>>) -> Json<Result<(), String>> {
    // let mut node = node_config.inner().0.lock().await;
...
#[get("/prompt")]
pub async fn get_mining_prompt(node_config: &State<NodeConfig>) -> Json<Option<MiningPrompt<ContentType>>> {
    // let node = node_config.inner().0.lock().await;
...
pub async fn set_content(node_config: &State<NodeConfig>, content: Json<ContentType>) {
